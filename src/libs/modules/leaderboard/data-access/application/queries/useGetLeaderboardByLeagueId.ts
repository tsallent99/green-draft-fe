...
type UseGetLeaderboardByLeagueIdOptions = {
	leagueId: number;
	config?: Omit<
		UseQueryOptions<LeaderboardDetailedEntity, Error>,
		"queryKey" | "queryFn"
	>;
};
...
export function useGetLeaderboardByLeagueId({
	...
	const queryKey = useMemo(
		() => createKeysGetLeaderboardByLeagueId(leagueId),
		[leagueId],
	);

	const { data, error, refetch, isFetching } = useQuery<
		LeaderboardDetailedEntity,
		Error
	>({
		...config,
		queryKey,
		queryFn: async () => {
			const leaderboardData =
				await repository.getLeaderboardByLeagueId(leagueId);
